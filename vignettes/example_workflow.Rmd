...
**chrongler** is an R-package that contains functions meant to re-format archaeological or historical "periods" stored as categorical variables. Using a concordance provided by the user, the periods can be replaced with absolute values taken from that concordance. This vignettes comments and illustrate the functions chrongler provides.
